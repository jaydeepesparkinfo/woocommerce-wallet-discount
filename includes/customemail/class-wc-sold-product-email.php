 ...
 * @extends \WC_Email
 */
class WC_Sold_Product_Email extends WC_Email {
    ...
    function __construct() {
            ...
            $this->id             = 'esbwoodemo_sold_product';
            $this->customer_email = true;
            $this->title          = __( 'Sold Your Product', 'esbwoodemo' );
            $this->description    = __( 'Your product has sold.', 'esbwoodemo' );

            $this->heading        = __( 'Sold Product', 'esbwoodemo' );
            $this->subject        = __( 'Your Product {site_title} from {order_date} is Sold', 'esbwoodemo' );

            $this->template_html  = 'emails/sold-product-html.php';
            $this->template_plain = 'emails/plain/sold-product-plain.php';
    ...
    public function trigger( $order_id ) {
        if ( $order_id ) {
                ...
                foreach ($items_info as $item_value) {
                   $product_aid  = $item_value['item_meta']['_product_id'];
                   $this->recipient = get_the_author_meta( 'user_email',  get_post_field( 'post_author', $product_aid[0] ));
                }
        }
